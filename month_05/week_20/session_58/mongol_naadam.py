...
DB_PASSWORD = "postgres" # энд өөрийн PostgreSQL нууц үгийг оруулна
DB_HOST = "localhost"
DB_PORT = "5432"
def get_connection():
...
def create_database():
    ...
    cursor = conn.cursor()
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS evevts(
        id SERIAL PRIMARY KEY,
        sport VARCGAR(255) NOT NULL,
        location VARCHAR(255)
        winner_id INTEGER,
        FOREIGN KEY (winner_id ReFERCES participants (id)           
           );

'''
    )
    cursor.execute('''
 # TODO events хүснэгтийг үүсгэх ''')
    conn.commit()
    cursor.close()
    conn.close()
...
def add_participant():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO participants (name, age , spots) VALUES (%s, %s, %s)",
        (name, age, sport)
    )
    conn.commit()
    cursor.close()
...
def record_winner():
    ...
    cursor.execute("SELECT id ,name, age FROM participants WHERE sport =%s", (event_id))
...
def view_events():
    conn = get_connection()
    cursor = conn.cursor()
    query= """SELECT e.id, e.sport, e.location, p.name FROM events e LEFT JOIN particpants p ON e.winner=p.id
"""
